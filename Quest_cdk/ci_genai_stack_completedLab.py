...
class CdkappStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        ec2_instance = ec2.Instance(self, "MyInstance",
            ...
            #ROLE ALREADY EXIST IN CONSOLE IN THIS CASE. Args definition: (self, arbitraryInstanceProfileName, roleName)
            role = iam.Role.from_role_name(self, "ec2_instance_role", "ec2_instance_role") 
        )
        # # create a for an ec2 instance using cdk with pyhton language
        # # Create IAM Role for EC2
        # my_ec2_role = iam.Role(self, "MyEc2Role",
        #     assumed_by=iam.ServicePrincipal("ec2.amazonaws.com"),
        #     description="IAM role for EC2 instance",
        # )

        # # Attach managed policy
        # my_ec2_role.add_managed_policy(
        #     iam.ManagedPolicy.from_aws_managed_policy_name("AmazonEC2FullAccess")
        # )
       

        # 2nd instance create a t3.micro ec2 instance for the web server in a private egress subnet and vpc.availability_zones[1]
        ec2_instance = ec2.Instance(self, "MyInstance_v2",
            instance_type = ec2.InstanceType("t3.micro"),
            machine_image = amzn_linux,
            vpc = vpc,
            vpc_subnets = ec2.SubnetSelection(subnet_type = ec2.SubnetType.PRIVATE_WITH_EGRESS),
